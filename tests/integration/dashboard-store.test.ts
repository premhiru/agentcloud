...
const other: TenantContext = { organizationExternalId: "org_other", userExternalId: "user_other", role: "owner", source: "demo" };
...
describe("tenant-scoped dashboard store", () => {
  it("never returns another tenant's workers", () => {
    ...
    expect(demoControlPlane.listWorkers(other)).toEqual([]);
  ...
  it("creates a compiled draft inside the authenticated tenant", async () => {
    ...
    expect(worker.organizationId).toBe("org_other");
