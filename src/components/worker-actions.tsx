...
import { FlaskConical, LoaderCircle, Pause, Play, Rocket } from "lucide-react";
...
export function WorkerActions({ workerId, status }: { workerId: string; status: string }) {
  ...
  async function act(action: "test" | "deploy" | "pause" | "resume") {
  ...
  return <div><div className="flex flex-wrap gap-2">
    ...
    {status === "DEPLOYED" && <button onClick={() => act("pause")} disabled={busy} className="button"><Pause size={16} />Pause</button>}
