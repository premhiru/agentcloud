...
const actionSchema = z.object({ action: z.enum(["deploy", "pause", "resume", "archive", "rollback", "test"]), versionId: z.string().uuid().or(z.string().startsWith("version_")).optional() }).strict();
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    if (parsed.data.action === "test") return NextResponse.json({ run: demoControlPlane.createPreviewRun(context, id) });
