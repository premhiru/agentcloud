...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try { const approval = await demoControlPlane.decideApproval(context, (await params).id, parsed.data.decision, parsed.data.comment); return NextResponse.json({ approval }); }
