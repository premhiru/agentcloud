          ...
          <div className="grid gap-5 lg:grid-cols-[1.25fr_.75fr]">
            ...
            <div className="space-y-5">
              <div className="card p-6">
                ...
                {readiness.requiredProviders.length ? <div className="mt-4 space-y-3">{readiness.requiredProviders.map((provider) => <div key={provider.provider} className="flex items-center justify-between gap-3 rounded-2xl border border-[var(--line)] p-3"><div><p className="font-extrabold">{providerLabel(provider.provider)}</p><p className="muted mt-1 text-xs">{provider.capabilities.length} capabilities</p></div><StatusBadge status={provider.connectionStatus} /></div>)}</div> : <p className="muted mt-4 text-sm">No external provider is required by this version.</p>}
