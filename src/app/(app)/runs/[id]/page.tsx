...
export default async function RunDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params; const context = await requireTenantContext(); const run = demoControlPlane.getRun(context, id); if (!run) notFound();
  const worker = demoControlPlane.getWorker(context, run.workerId);
  return <div className="mx-auto max-w-4xl"><Link href={`/workers/${run.workerId}`} className="muted inline-flex items-center gap-2 text-sm font-bold"><ArrowLeft size={16} />Back to worker</Link><div className="mt-6 flex flex-wrap items-start justify-between gap-4"><div><p className="eyebrow">{run.mode === "dry_run" ? "Safe test" : "Live run"}</p><h1 className="mt-2 text-3xl font-black">{worker?.name ?? "Worker run"}</h1><p className="muted mt-2">Run {run.id.slice(0, 8)} · WorkerSpec pinned to {run.workerVersionId.slice(0, 12)}</p></div><StatusBadge status={run.status} /></div>
