...
export default async function ApprovalsPage() {
  const context = await requirePageTenantContext(); const controlPlane = await getControlPlane(); const approvals = await controlPlane.listApprovals(context); const workers = await controlPlane.listWorkers(context); const names = new Map(workers.map((worker) => [worker.id, worker.name]));
