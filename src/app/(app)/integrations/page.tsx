...
export default async function IntegrationsPage() {
  const demo = isDemoMode(); const connected = await (await getControlPlane()).listConnections(await requireTenantContext()); const statuses = new Map(connected.map((item) => [item.provider, item]));
