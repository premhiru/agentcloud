...
export async function executeMcpTool(name: McpToolName, input: Record<string, unknown>, context: TenantContext): Promise<Record<string, unknown>> {
  ...
  switch (name) {
    ...
    case "test_worker": { const run = demoControlPlane.createPreviewRun(context, workerId); return { runId: run.id, status: run.status, mode: run.mode }; }
    ...
    case "approve_action": return { approval: demoControlPlane.decideApproval(context, String(input.approvalId ?? ""), "approve", typeof input.comment === "string" ? input.comment : undefined) };
    case "reject_action": return { approval: demoControlPlane.decideApproval(context, String(input.approvalId ?? ""), "reject", typeof input.comment === "string" ? input.comment : undefined) };
    ...
    case "trigger_worker": { const run = demoControlPlane.createLiveRun(context, workerId); return { runId: run.id, status: run.status }; }
